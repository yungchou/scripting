...
Set-AzKeyVaultAccessPolicy -VaultName $keyVaultName -PermissionsToKeys unwrapKey -PermissionsToSecrets get -UserPrincipalName $currentUser;
...
if ($kekUrl) {
  ...
  $platformParam = New-Object "Microsoft.IdentityModel.Clients.ActiveDirectory.PlatformParameters" -ArgumentList 0
  # Acquire token
  $authResult = $authContext.AcquireTokenAsync($resourceAppIdURI, $clientId, $redirectUri, $platformParam).GetAwaiter().GetResult()
  ...
  $jsonObject = "{'alg' : 'RSA-OAEP', 'value' : '$secretBase64'}"

...
<#
https://github.com/AsrOneSdk/published-scripts/blob/master/CopyKeysSinglePass-Az.ps1

1. Execute Copy-keys.ps1.
2. Provide Azure credentials to sign in.
3. Select the Azure subscription of your VMs.
4. Wait for the resource groups to load, 
   then select the Resource group of your VMs.
5. Select the VMs from the list that's displayed. 
   Only VMs enabled for disk encryption are on the list.
6. Select the Target location.
   - Disk encryption key vaults
   - Key encryption key vaults

By default, Site Recovery creates a new key vault in the target region. 
The vault's name has an "asr" suffix that's based on the source VM disk 
encryption keys. If a key vault already exists that was created by Site Recovery, 
it's reused. Select a different key vault from the list if necessary.
#>
